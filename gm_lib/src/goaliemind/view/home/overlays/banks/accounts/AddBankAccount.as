package goaliemind.view.home.overlays.banks.accounts 
{
	 ...
	 * (c) 2015
	 */
	public class AddBankAccount extends HomePanelScreen
	{
		...
		{
			LogUtil.log("AddBankAccount.onBackButton()");
		...
		{
			LogUtil.log("AddBankAccount.onActionButton()");
		...
		private function createList():void
		{
			...
			var items:Array = [];
			items[items.length] = { label:"Routing Number", maxChars:100, icon:null };
			items[items.length] = { label:"Account Number", maxChars:100, icon:null };
			items[items.length] = { label:"Confirm Account Number", maxChars:100, icon:null };
			items.fixed = true;
