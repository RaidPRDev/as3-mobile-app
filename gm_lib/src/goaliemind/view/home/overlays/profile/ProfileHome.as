package goaliemind.view.home.overlays.profile 
{
	 ...
	 * (c) 2015
	 */
	public class ProfileHome extends HomePanelScreen
	{
		...
		{
			LogUtil.log("ProfileHome.onAddButton()");
			
			this.dispatchEventWith("addProfileScreen");
		}	
		
		private function onBackButton(e:Event):void 
		{
			LogUtil.log("ProfileHome.onBackButton()");
		...
		{
			var items:Array = [];
			items[items.length] = { label:"Juancito Torres", isEnabled:true, icon:null, eventName:"account" };
			items.fixed = true;
